...
class GerenciadorNotasApp:
    ...
    def editar_notas_disciplina_top_level(self, aluno_id, disciplina_item, parent_window):
        ...
        # --- Seção para Adicionar/Remover Componentes ---
        ...
        def adicionar_componente():
            ...
            if not novo_nome:
                ...
                return
            # Verifica se já existe um componente com este nome (case-insensitive)
            ...
            if novo_nome.lower() in componentes_existentes:
                messagebox.showwarning("Aviso", f"O componente '{novo_nome}' já existe.")
                return

            tree.insert("", "end", values=(novo_nome, "0,0"))
            novo_comp_entry.delete(0, tk.END)
            messagebox.showinfo("Sucesso", f"Componente '{novo_nome}' adicionado com nota inicial 0,0. Lembre-se de salvá-lo.")
        ...
        def remover_componente():
            ...
            if messagebox.askyesno("Confirmar Remoção", f"Tem certeza que deseja remover o componente '{nome_comp}'?\nEsta ação é irreversível após salvar.", parent=top):
                tree.delete(sel)
                messagebox.showinfo("Sucesso", f"Componente '{nome_comp}' removido (será salvo após clicar em 'Salvar Edição').")

        ttk.Button(comp_frame, text="➕ Adicionar Componente", command=adicionar_componente).pack(side="left", padx=10)
        ttk.Button(comp_frame, text="➖ Remover Componente Selecionado", command=remover_componente, style="Danger.TButton").pack(side="left", padx=10)

        # --- Botão Salvar Principal ---
        def salvar_edicao():
            novos_comps = []
            if not tree.get_children():
                messagebox.showwarning("Aviso", "Não é possível salvar uma disciplina sem componentes de nota.")
                return
    ...
    def visualizar_notas_gui(self, aluno_id_param=None):
        ...
        def exportar_para_impressao():
            ...
            # Gerar conteúdo HTML (omitido para brevidade, pois não mudou)
            html = f"""
            <html>
            <head>
