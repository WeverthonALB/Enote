...
class GerenciadorNotasApp:
    ...
    def criar_tabelas(self):
        ...
        """)
        # Adicionado 'pk_id' para ser a chave primária real da linha de nota
        self.cursor.execute("""
    ...
    def carregar_dados(self):
        ...
        # Carrega o pk_id do banco para a edição futura
        ...
        for pk_id, aluno_id, disciplina, comp_json in self.cursor.fetchall():
            ...
            # Armazena o pk_id junto com os dados da nota
    ...
    def adicionar_notas_gui(self):
        ...
        def atualizar_nota():
            try:
                ...
                return
            try:
                # Trata a entrada de nota, permitindo vírgula ou ponto
            ...
            # Exibe a nota formatada com vírgula e 1 casa decimal
        ...
        def salvar():
            ...
            # Insere no banco (pk_id é AUTOINCREMENT)
            self.cursor.execute("INSERT INTO notas (aluno_id, disciplina, componentes) VALUES (?, ?, ?)",
                                 (aluno_id, disc, comp_json))
            pk_id = self.cursor.lastrowid # Pega o ID gerado automaticamente
            self.conexao.commit()
            # Adiciona o pk_id nos dados em memória
        ...
        ttk.Button(frame, text="Salvar Disciplina", command=salvar).pack(pady=10)
    ...
    def editar_notas_gui(self):
        ...
        # Mapeia o item do Treeview para o item de nota completo (incluindo pk_id)
        tree_map = {} 
        for item in notas_aluno:
            media = self.calcular_media_por_disciplina(item)
            media_str = self.formatar_numero(media)
            # Insere o ID do Treeview e mapeia para o item completo
        ...
        def abrir_edicao():
            ...
            # Chama a função que abre a janela de edição de componentes
    ...
    def editar_notas_disciplina_top_level(self, aluno_id, disciplina_item, parent_window):
        ...
        componentes_atuais = disciplina_item['componentes']
        aluno_nome = self.alunos[aluno_id]['nome']
        
        top = tk.Toplevel(self.root)
        top.title(f"Editar Notas de {disciplina} - {aluno_nome}")
        top.geometry("700x500")

        frame = ttk.Frame(top, padding=20, style="Content.TFrame")
        frame.pack(expand=True, fill='both')
        
        ttk.Label(frame, text=f"Editando Notas de: **{disciplina}**", 
        ...
        ttk.Button(frame, text="Atualizar Nota no Componente", command=atualizar_nota).pack(pady=5)
        ...
        def salvar_edicao():
            ...
            messagebox.showinfo("Sucesso", f"Notas de {disciplina} atualizadas!")
            top.destroy()
            parent_window.destroy() # Fecha a janela de seleção de disciplina

        ttk.Button(frame, text="Salvar Edição", command=salvar_edicao).pack(pady=10)
    ...
    def calcular_media_por_disciplina(self, nota_item):
        ...
        # Certifica-se de que há notas antes de dividir
    ...
    def calcular_media_gui(self, aluno_id=None, show_message=True):
        ...
        # Calcula a média da média das disciplinas
    ...
    def visualizar_notas_gui(self, aluno_id_param=None):
        ...
        def exportar_para_impressao():
            ...
            # Gerar conteúdo HTML (mesmo código original)
            html = f"""
            <html>
            <head>
            ...
            # Adicionar notas das disciplinas
            for item in notas_aluno:
                media = self.calcular_media_por_disciplina(item)
                media_str = self.formatar_numero(media)
                html += f"<tr><td>{item['disciplina']}</td><td>{media_str}</td></tr>\n"
            
            # Calcular e adicionar média geral
            ...
            # Salvar o arquivo e abrir no navegador
            try:
                ...
                messagebox.showinfo("Sucesso", 
                                     "Boletim exportado!\n\nO arquivo foi aberto no seu navegador. Use a função de impressão (Ctrl+P) do navegador.", 
                                     parent=top)
